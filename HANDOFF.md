...
- 4 个原创通用分享小图 × 4 条通用文案映射为 16 个静态入口 `a/` 至 `p/`，微信抓取可直接读取对应 OG 信息。
...
```text
https://markjinli.github.io/hx/{a-p}/#2.<base64url(七字段 UTF-8 数据)>.<6字符 CRC32>
```
...
旧版 `#i=v1.<base64url(JSON)>.<8字符CRC32>` 仍能解码。16 个旧 `/share/v2/{icon-id}/{copy-id}/` 地址由兼容站映射到新 `a/` 至 `p/`。
...
- `share-options.js`：4 个图标、4 条文案和固定 `a` 至 `p` 路由映射。
...
6. 微信可能缓存旧卡片；用未发送过的组合路径测试，不要把缓存误判为代码未生效。
...
修改 `index.html`、音乐、图标或分享选项后先运行 `npm run build`。发布需把 `a/` 至 `p/` 生成页一并提交并推送 `main`。
